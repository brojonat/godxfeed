...
func handleGetSymbolRegexpTimeseriesData(s service.Service) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("symbol_only") == "true" {
			// fetch just the symbol data
		...
		// development, but the format shouldn't change in general, it should surface
		// a list of rows of timestamped bid/ask prices and sizes for the symbol.
		rows, err := s.DBQ().GetSymbolDataRaw(r.Context(), dbgen.GetSymbolDataRawParams{
			Symregexp: symregexp,
			TsStart:   pgtype.Timestamptz{Time: time.Time{}, Valid: true},
			TsEnd:     pgtype.Timestamptz{Time: time.Now(), Valid: true},
